...
public class Vigenere {
	...
	public static void main(String[] args) {
		...
		char[] encrypted = encrypt(satz_char, key_list);
		
		System.out.println(encrypted);
	
		//decrypt(encrypted, key_list);
	...
	/** 
	 * @param satz      The sentence being encrypted  
	 * @param key       The key used for the encryption
	 * @return char[]   Returns an encrypted sentence
	*/
	public static char[] encrypt(char[] satz, char[] key) {
	...
	/*************************************************************************************/
	/*
	 * TODO: Decryption doesn't work with only flipping + 
	 * Dieser Code-Teil würde rein logisch genau das machen, was er soll, heißt entschlüsseln. Macht er aber aus irgendeinem
	 * Grund nicht... Ich finde den Fehler nicht. Versucht habe ichs aber :/
	public static void decrypt(char[] satz, char[] key) {
		
		for (int i = 0; i < satz.length; i++) {
			if ((int) satz[i] >= 65 && (int) satz[i] <= 90) {
				if ((int) key[i%key.length] >= 65 && (int) key[i%key.length] <= 90) {
					satz[i] = (char) ((((((int) satz[i]) - 65) - (((int) key[i%key.length]) - 65)) % 26) + 65);
				} else {
					satz[i] = (char) ((((((int) satz[i]) - 65) - (((int) key[i%key.length] - 32) - 65)) % 26) + 65);
				}
			} else {
				if ((int) key[i%key.length] >= 65 && (int) key[i%key.length] <= 90) {
					satz[i] = (char) ((((((int) satz[i]) - 97) - (((int) key[i%key.length] + 32) - 97)) % 26) + 97);
				} else {
					satz[i] = (char) ((((((int) satz[i]) - 97) - (((int) key[i%key.length]) - 97)) % 26) + 97);	
				}
			}
		}
		
		String out = String.valueOf(satz);
